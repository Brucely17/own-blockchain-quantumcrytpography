class ValidatorPool {
    ...
    /**
     * ✅ Registers a new validator using their wallet public key.
     * - Ensures uniqueness in the pool.
     */
    ...
    /**
     * ✅ Automatically selects validators based on reputation & workload.
     * - Prioritizes validators with **high reputation & low workload**.
     */
    selectValidators(transactionId, count = 3) {
      const sortedValidators = Object.keys(this.validators)
        .sort((a, b) => {
          ...
          if (repDiff === 0) {
            // ✅ If reputation is the same, prefer validators with fewer assigned transactions.
            return this.validators[a].assignedTransactions - this.validators[b].assignedTransactions;
          }
          return repDiff;
        });
  
      // ✅ Select top `count` validators
    ...
    /**
     * ✅ Updates validator reputation based on decision accuracy.
     * - Rewards or penalizes validators based on correct/incorrect approvals.
     */
    updateReputation(validatorId, isAccurate) {
      if (!this.validators[validatorId]) return;
  
      if (isAccurate) {
        this.validators[validatorId].reputation += 5; // ✅ Reward for correct validation
